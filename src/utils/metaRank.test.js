...
describe('TOTAL_TIERS', () => {
  it('is derived from the config (11 categories × 4 tiers today)', () => {
    expect(TOTAL_TIERS).toBe(44);
  });
});
...
describe('countUnlockedTiers', () => {
  it('sums tiers reached across categories', () => {
    ...
    const account = { gamesCount: 150, breakdown: { city: 26000 } };
    expect(countUnlockedTiers(account)).toBe(6);
  });

  it('is 0 for a fresh account', () => {
    expect(countUnlockedTiers({ gamesCount: 0, breakdown: {} })).toBe(0);
  });
});
